...
def test(n_envs=10, task=None, observations=None,
         vis_gen=_VIS_GEN, visual_dist='baseline',
         show=False, iterations=100):

    logger.info(f'Testing environment with variables task={task}, '
    ...
    try:
        ...
        for i in range(n_envs):
            rollout(
                mdp, observation=o_render, show_render=show,
                label=task+str(i))
    except Exception:
        traceback.print_exc()
        logger.error('[FAILED]')
        raise ValueError(f'Exception raised on task {task} and observations'
                         f' {observations}.')
    logger.info('[PASSED]')
...
if __name__ == '__main__':
    for task in _TASKS:
        for observations in _OBSERVATIONS:
            test(task=task, observations=observations, vis_gen=_VIS_GEN,
                 show=True)
