    ...
    def __init__(
        ...
        if task_name == "empty":
            ...
            if task_distr == "easy":
                init_config = {
                    "numbers": [
                        (ThingFactory([Charger, Magnet, Bumper, Damper,
                                       Ball]), k),
                        (GoalTile, 1),
                        (GolfBall, 1),
                    ],
                ...
                init_config = {
                    ...
                    "priors": [
                        ...
                        Prior(Size,
                              ...
                              entity_type=Tile),
                        Prior(Charge,
                              GaussianMixtureNoise(means=[-1.0, 1.0],
                                                   stds=[0.1, 0.1]),
                              entity_type=Charger),
                        Prior(Magnetism,
                              GaussianMixtureNoise(means=[-1.0, 1.0],
        ...
        config = dict(max_steps_per_episode=max_steps,
...
class SEGARSingleEnv(SEGAREnv):
    ...
    def __init__(
        ...
        self.observation_space = Box(shape=(resolution, resolution,
                                            3 * framestack),
                                     low=0,
                                     high=255,
                                     dtype=np.uint8)
        self.action_space = Box(-self.action_max,
                                self.action_max,
                                shape=(2, ),
                                dtype=np.float32)
...
if __name__ == "__main__":
    ...
    def check_same_env(env1, env2):
        ...
        for obj in objs:
            ...
            factors_level2 = [
                x.value for x in env2.env.envs[0].mdp_list[0].env.env.sim.
                things[obj].factors.values()
                if type(x.value) == float or type(x.value) == np.ndarray
            ]
            names = np.array([
                y for y, x in env1.env.envs[0].mdp_list[0].env.env.sim.
    ...
    def check_same_process(env1):
        ...
        for obj in objs:
            ...
            factors_level2 = [
                x.value for x in env1.env.envs[1].mdp_list[0].env.env.sim.
                things[obj].factors.values()
                if type(x.value) == float or type(x.value) == np.ndarray
            ]
            names = np.array([
    ...
    def check_same_level(env1):
        ...
        for obj in objs:
            ...
            names = np.array([
                y for y, x in env1.env.envs[0].mdp_list[1].env.env.sim.
                things[obj].factors.items()
                if type(x.value) == float or type(x.value) == np.ndarray
            ])
            ...
            for f1, f2, f1_2, name in zip(factors_level1, factors_level2,
                ...
                if not np.all(f1 == f1_2):
                    print("[%s] Factor `%s` is different for level 0" %
                          (obj, name))
    ...
    env1 = SEGAREnv("emptyx0-hard-rgb",
                    ...
                    max_steps=200,
                    seed=123)
    env2 = SEGAREnv("emptyx0-hard-rgb",
                    num_envs=1,
                    num_levels=5,
                    framestack=1,
                    resolution=64,
