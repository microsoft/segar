...
class GoalTile(Tile, default={Label: 'goal',
                              Order: -2,
                              Shape: Circle(0.3),
                              Text: 'G',
                              ID: 'goal'}):
...
puttputt_default_config = {
    ...
    'priors': [
        ...
        Prior(Size, GaussianNoise(0.2, 0.01, clip=(0.1, 0.3)),
              entity_type=Object),
        Prior(Size, GaussianNoise(1.0, 0.01, clip=(0.5, 1.1)),
              entity_type=Tile),
...
puttputt_random_middle_config = {
    'numbers': [
        (ThingFactory([Charger, Magnet, Bumper, Damper, Object, SandTile,
                       MagmaTile, Hole, FireTile]), 1),
        (GoalTile, 1),
        (GolfBall, 1)
    ],
    'priors': [
...
invisiball_config = {
    ...
    'priors': [
        ...
        Prior(Position, RandomBottomLocation(), entity_type=GolfBall),
        Prior(Position, RandomTopLocation(), entity_type=GoalTile),
        Prior(Size, GaussianNoise(0.2, 0.01, clip=(0.1, 0.25)),
              entity_type=Object),
        Prior(Mass, 1.0),
        Prior(Mobile, True),
        Prior(Charge, 1.,  entity_type=Charger)
    ]
}
...
class PuttPuttInitialization(ArenaInitialization):
    ...
    def __init__(self, config: dict = None):
        ...
        if not has_golfball:
            raise KeyError(f'`{GolfBall}` number must be defined in this '
                           'initialization.')
        if not has_goal:
            raise KeyError(f'`{GoalTile}` number must be defined in this '
                           'initialization.')
    ...
    def set_arena(self, init_things: Optional[list[Entity]] = None) -> None:
        ...
        if self.goal_id is None:
            raise RuntimeError(f'Goal was not set in arena. Config is '
                               f'{self._config} and params are '
                               f'{self._numbers}')

        if self.golfball_id is None:
            raise RuntimeError(f'Golfball was not set in arena. Config is '
                               f'{self._config} and params are '
                               f'{self._numbers}.')
...
class PuttPutt(Task):
    ...
    """
    def __init__(self, initialization: PuttPuttInitialization,
        ...
        super().__init__(action_range=action_range, action_shape=action_shape,
    ...
    def golfball_id(self) -> ID:
        if not hasattr(self._initialization, 'golfball_id'):
            raise AttributeError('Initialization must define `golfball_id` to '
    ...
    def goal_id(self) -> ID:
        if not hasattr(self._initialization, 'goal_id'):
            raise AttributeError('Initialization must define `goal_id` to '
                                 'be compatible with task.')
    ...
    def done(self, state: dict) -> bool:
        ...
        under_vel = bool((self._max_ball_at_goal_velocity is None) or (
            ball_state[Velocity].norm() <= self._max_ball_at_goal_velocity))
    ...
    def results(self, state: dict) -> dict:
        ...
        distance = self.sim.l2_distance(self.golfball_id, self.goal_id)
        ball_state = state['things'][self.golfball_id]
        return dict(dist_to_goal=distance,
