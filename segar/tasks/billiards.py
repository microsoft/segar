...
def billiard_ball_positions(start: list[float, float],
                            r: float = _DEFAULT_BALL_SIZE / 2 + 1e-3,
                            n: int = 10) -> list[list[float, float]]:
    x, y = start
    sq2r = math.sqrt(2.) * r
    positions = [start]
    positions += [[x - sq2r, y + sq2r],
                  [x + sq2r, y + sq2r]]
    positions += [[x - 2 * sq2r, y + 2 * sq2r],
                  [x, y + 2 * sq2r],
                  [x + 2 * sq2r, y + 2 * sq2r]]
    positions += [[x - 3 * sq2r, y + 3 * sq2r],
                  [x - sq2r, y + 3 * sq2r],
                  [x + sq2r, y + 3 * sq2r],
                  [x + 3 * sq2r, y + 3 * sq2r]]
...
class CueBall(Object, default={Label: 'cueball',
...
billiards_default_config = {
    ...
    'priors': [
        Prior(Size, GaussianNoise(_DEFAULT_BALL_SIZE, 0.01,
                                  clip=(_DEFAULT_BALL_SIZE / 2.,
                                        3 * _DEFAULT_BALL_SIZE / 2.)),
              entity_type=CueBall),
...
class BilliardsInitialization(ArenaInitialization):
    ...
    def sample(self, max_iterations: int = 100) -> list[Entity]:
        ...
        ball_positions = billiard_ball_positions([0., 0.])

        for i, pos in enumerate(ball_positions):
            ball = Ball({Position: pos, Text: f'{i + 1}', ID: f'{i + 1}_ball'})
            sampled_things.append(ball)

        hole_positions = [[-.9, -.9], [-.9, .9], [.9, -.9], [.9, .9]]
        for i, pos in enumerate(hole_positions):
            hole = Hole({Position: pos, ID: f'{i}_hole',
                         Size: _DEFAULT_HOLE_SIZE})
...
class Billiards(Task):
    ...
    """
    def __init__(self, initialization: BilliardsInitialization,
    ...
    def cueball_id(self) -> ID:
        if not hasattr(self._initialization, 'cueball_id'):
            raise AttributeError('Initialization must define `cueball_id` to '
    ...
    def hole_ids(self) -> list[ID]:
        if not hasattr(self._initialization, 'hole_ids'):
            raise AttributeError('Initialization must define `hole_ids` to '
                                 'be compatible with task.')
    ...
    def ball_ids(self) -> list[ID]:
        if not hasattr(self._initialization, 'ball_ids'):
            raise AttributeError('Initialization must define `ball_ids` to '
                                 'be compatible with task.')
    ...
    def reward(self, state: dict) -> float:
        ...
        for ball_id in self.ball_ids:
            distance = min([self.sim.l2_distance(ball_id, hole_id) for
                            hole_id in self.hole_ids])
