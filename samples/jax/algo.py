...
def mse_loss(params: flax.core.frozen_dict.FrozenDict, apply_fn: Callable, X: jnp.ndarray, y: jnp.ndarray):
    X = jax.lax.stop_gradient(X) # prevent latent factors to backprop into representation
...
def loss_actor_and_critic(params_model: flax.core.frozen_dict.FrozenDict,
                          ...
                          action: jnp.ndarray, latent_factors: jnp.ndarray, clip_eps: float,
                          critic_coeff: float,
                          entropy_coeff: float,
...
def select_action(
    ...
    value, pi_dist = train_state.apply_fn(train_state.params, state, latent_factors)
...
def update_ppo(train_state: TrainState, batch: Tuple, num_envs: int,
    ...
    total_loss, (value_loss, loss_actor, ent, value_pred, target_val, gae_val, log_prob) = total_loss
