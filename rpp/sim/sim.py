...
class Simulator:
    ...
    def __init__(
        ...
        :param state_buffer_length: Length of state buffer, e.g.,
            for stacking.
        :param auto_adopt: When object is created, adopt automatically.
    ...
    def reset(self) -> None:
        ...
        with self.auto_adopt(True):
            Entity({Friction: self.parameters[Friction],
                    ID: 'global_friction'})
    ...
    def _get_next_id(self, unique_id: ThingID = None) -> ThingID:
        """Get next key (id) for thing.

        :param unique_id: Optional unique id for thing
        :return: The ID.
        """
        ...
        if not isinstance(unique_id, (int, str, ID)):
            raise TypeError(unique_id)
        if isinstance(unique_id, ID):
            unique_id = unique_id.value
        if isinstance(unique_id, str) and unique_id.isdigit():
            unique_id = int(unique_id)
        return str(unique_id)

    def get_new_id(self, thing: Entity, thing_id: Optional[ThingID] = None
                   ) -> ThingID:
        ...
        thing_id = self._get_next_id(thing_id)
        if self._auto_adopt:
            return self.adopt(thing, thing_id)
        else:
            return thing_id
    ...
    def adopt(self, thing: Entity, thing_id: Union[ThingID, ID] = None
              ) -> ThingID:
        ...
        if thing_id is None:
            if ID in thing:
                thing_id = thing[ID].value
            else:
                thing_id = self.get_new_id(thing)

        if thing_id in self._things:
            raise KeyError(f'{thing_id} cannot be used as `unique_id` '
                           f'because it already is in use.')

        if isinstance(thing_id, str) and thing_id.isdigit():
            thing_id = int(thing_id)

        with thing.in_place():
            thing[ID] = ID(thing_id)

        self._things[thing_id] = thing
        return thing_id

    class _AutoAdopt:
        """For auto-adoption of Things as they are created.

        Things are not automatically adopted by the sim. This class enables
            this functionality when needed.

        """
        def __init__(self, sim: Simulator, value: bool):
            self.prior_adopt = sim._auto_adopt
            self.sim = sim
            self.value = value

        def __enter__(self):
            self.sim._auto_adopt = self.value

        def __exit__(self, exc_type, exc_val, exc_tb):
            self.sim._auto_adopt = self.prior_adopt

    def auto_adopt(self, value: bool = True) -> _AutoAdopt:
        """For auto-adopting things.

        Normally, creating things wont add them to the sim. However,
        ...
        :return:
        """
        return self._AutoAdopt(self, value)
    ...
    def add_object(self,
        ...
        o = Object(initial_factors, unique_id=unique_id)
        if not self._auto_adopt:
            self.adopt(o)
        return o[ID].value
    ...
    def add_tile(self,
        ...
        o = Tile(initial_factors, unique_id=unique_id)
        if not self._auto_adopt:
            self.adopt(o)
        return o[ID].value
    ...
    def add_sand(self,
        ...
        o = SandTile(initial_factors, unique_id=unique_id)
        if not self._auto_adopt:
            self.adopt(o)
        return o[ID].value
    ...
    def add_magma(self,
        ...
        o = MagmaTile(initial_factors, unique_id=unique_id)
        if not self._auto_adopt:
            self.adopt(o)
        return o[ID].value
    ...
    def add_fire(self,
        ...
        o = FireTile(initial_factors, unique_id=unique_id)
        if not self._auto_adopt:
            self.adopt(o)
        return o[ID].value
    ...
    def add_hole(self,
        ...
        o = Hole(initial_factors, unique_id=unique_id)
        self.adopt(o)
        return o[ID].value
    ...
    def add_bumper(self,
        ...
        o = Bumper(initial_factors, unique_id=unique_id)
        if not self._auto_adopt:
            self.adopt(o)
        return o[ID].value
    ...
    def add_damper(self,
        ...
        o = Damper(initial_factors, unique_id=unique_id)
        if not self._auto_adopt:
            self.adopt(o)
        return o[ID].value
    ...
    def add_charger(self,
        ...
        o = Charger(initial_factors, unique_id=unique_id)
        if not self._auto_adopt:
            self.adopt(o)
        return o[ID].value
    ...
    def add_magnet(self,
        ...
        o = Magnet(initial_factors, unique_id=unique_id)
        if not self._auto_adopt:
            self.adopt(o)
        return o[ID].value
